(function ($) {
    $.fn.data_table = function (options) {
        ...
        var _this_selector = this.selector;
        var type_aheads = {};
        var search_data = {};
        ...
        s.init = function ()
        {
            ...
            $(_this).on("click", ".pagination a", function (e) {
                e.preventDefault();
                _this = $(this).closest(_this_selector);
        ...
        s.setup_search = function ()
        {
            ...
            if (has_search) {
                ...
                $("thead tr:first th", _this).each(function () {
                    ...
                    var search_s;
                    var search_s_hdn;
                    if (typeof search != "undefined" || typeof s.dropdowns[search_name] != "undefined")
                    {
                        var last_v = "";
                        var last_v_id = "";
                        ...
                        if(typeof s.dropdowns[search_name] != "undefined"){
                            search_s = s.make_drop_down(s.dropdowns[search_name], search_name);
                            //search_inp.append(dropdown_html);
                        }
                        if (search == "true") {

                        }

                        if (search == "typeahead" || typeof s.dropdowns[search_name] != "undefined") {
                ...
                $("thead tr:eq( 1 ) select.type_dropdown", _this).unbind("change", function () {
                });
                $("thead tr:eq( 1 ) select.type_dropdown", _this).bind("change", function (e) {
                    s.fetch_inputs_selects();
                    s.url = s.def_url;
                    s.trigger();
                });
                ...
                $("thead tr:eq( 1 ) input", _this).bind("keyup", function (e) {
                    ...
                    if (code == 13) {
                        e.preventDefault();
                        s.fetch_inputs_selects(this);
                        s.url = s.def_url;
                        s.trigger();
                    }
                });
                
                $("thead tr:eq( 1 ) input", _this).unbind("change", function () {
                });
                
                $("thead tr:eq( 1 ) input", _this).bind("change", function (e) {
                    e.preventDefault();
        ...
        s.make_drop_down = function(d_data, elem){
            var html = "<select class='form-control type_dropdown' name='search_"+elem+"' id='search_"+elem+"'><option value=''>Select</option>";
            for(var i in d_data){
                var obj = d_data[i];
        ...
        s.fetch_inputs_selects = function(inp_this){
            var this_id = $(inp_this).attr("id");
            var this_val = $.trim($(inp_this).val());
            if (this_val.length == 0) {
                $("#" + this_id + "_id").val("");
            }
            search_data = {};
            ...
            $("thead tr:eq( 1 ) select.type_dropdown", _this).each(function (i) {
                var value = 0;
                var value = $(this).val();
                var name = $(this).attr("name");
                name = name.replace("search_", "");
